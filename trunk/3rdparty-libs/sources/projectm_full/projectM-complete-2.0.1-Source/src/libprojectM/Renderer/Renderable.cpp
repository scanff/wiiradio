...
void DarkenCenter::Draw(RenderContext &context)
{
		...
		WII_VertArrays(GX_TRIANGLEFAN,&points[0][0],0,&colors[0][0],0,6,2);
#else
...
void Shape::Draw(RenderContext &context)
{
	...
	float t;
            ...
            if ( additive==0)  GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA, GX_BL_INVSRCALPHA, GX_LO_CLEAR);
			else    GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA, GX_BL_ONE, GX_LO_CLEAR);
#else
...
#else
                    int tex = context.textureManager->getTexture(imageUrl);
					if (tex != 0)
                        WII_SetCurrentTexture(0);//tex;
                    else return;
#endif
				}
#ifdef _WII_
                Mtx copymtx;
                guMtxCopy(modelview,copymtx);
                context.aspectRatio=1.0;
...
#ifdef _WII_
                WII_VertArrays(GX_TRIANGLEFAN, &points[0][0], &tex[0][0],&colors[0][0],0,sides+2,2);
                guMtxCopy(copymtx,modelview);
                //guMtxIdentity(modelview);
#else
				glVertexPointer(2,GL_FLOAT,0,points);
				glColorPointer(4,GL_FLOAT,0,colors);
...
*/
...
#ifdef _WII_
              WII_VertArrays(GX_TRIANGLEFAN, &points[0][0], 0,&colors[0][0],0,sides+2,2);
#else
			  glVertexPointer(2,GL_FLOAT,0,points);
			  glColorPointer(4,GL_FLOAT,0,colors);
...
#ifdef _WII_
            if (thickOutline==1)  WII_SetLineWidth(context.texsize < 512 ? 1 : 2*context.texsize/512,GX_TO_ZERO);
#else
			...
			WII_SetColors(border_r * 0xff, border_g * 0xff, border_b * 0xff, (border_a * masterAlpha) * 0xff);
#else
			glColor4f( border_r, border_g, border_b, border_a * masterAlpha);
#endif
            //float points[sides+1][2];
            float points[sides+2][2]; // no line loop in GX so create the last coord as the first
            ...
            // From what I read the last coord in a line loop is connected to the first so this should
            // do it manually
            points[sides+1][0] = points[0][0];
            points[sides+1][1] = points[0][1];

            WII_VertArrays(GX_LINESTRIP, &points[0][0], 0,0,0,sides,2);

            if (thickOutline==1)  WII_SetLineWidth(context.texsize < 512 ? 1 : context.texsize/512,GX_TO_ZERO);
#else
			glVertexPointer(2,GL_FLOAT,0,points);
...
void MotionVectors::Draw(RenderContext &context)
{
    ...
    GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA, GX_BL_INVSRCALPHA, GX_LO_CLEAR);
    WII_SetPointSize(length);
	WII_SetColors(r * 0xff, g * 0xff, b * 0xff, (a * masterAlpha) * 0xff);

    WII_VertArrays(GX_POINTS,&points[0][0],0,0,0,size,2);
#else
	glVertexPointer(2,GL_FLOAT,0,points);
...
#ifdef _WII_
    GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA, GX_BL_INVSRCALPHA, GX_LO_CLEAR);
    WII_SetColors(outer_r * 0xff, outer_g * 0xff, outer_b * 0xff, (outer_a * masterAlpha) * 0xff);
#else
	glEnableClientState(GL_VERTEX_ARRAY);
	glDisableClientState(GL_COLOR_ARRAY);
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsA[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsB[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsC[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsD[0][0],0,0,0,4,2);

	WII_Rectd(of, of, of+iff, texof);
	WII_Rectd(of+iff, of, texof-iff, of+iff);
	WII_Rectd(texof-iff, of, texof, texof);
	WII_Rectd(of+iff, texof, texof-iff, texof-iff);
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsE[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsF[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsG[0][0],0,0,0,4,2);
#else
...
#ifdef _WII_
    WII_VertArrays(GX_TRIANGLESTRIP,&pointsH[0][0],0,0,0,4,2);
#else
