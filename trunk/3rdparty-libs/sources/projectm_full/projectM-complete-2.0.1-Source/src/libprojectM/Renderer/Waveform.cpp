...
void Waveform::Draw(RenderContext &context)
   {
		...
		//if (samples > 2048) samples = 2048;
			...
			if (thick)
			{
			  WII_SetLineWidth(context.texsize <= 512 ? 2 : 2*context.texsize/512,GX_TO_ZERO);
			  WII_SetPointSize(context.texsize <= 512 ? 2 : 2*context.texsize/512);

			}
			else WII_SetPointSize(context.texsize <= 512 ? 1 : context.texsize/512);
			...
			if (dots) WII_VertArrays(GX_POINTS,&p[0][0],0,&colors[0][0],0,samples,2);
			else  WII_VertArrays(GX_LINESTRIP,&p[0][0],0,&colors[0][0],0,samples,2);

			WII_SetPointSize(context.texsize < 512 ? 1 : context.texsize/512);
			WII_SetLineWidth(context.texsize < 512 ? 1 : context.texsize/512,GX_TO_ZERO);
