...
{
#ifdef _WII_
    // ... useful for debugging problems on wii stub
//    printf("Loading preset: %s\n",preset->_absoluteFilePath.c_str());
#endif
...
//      std::cerr << "token prefix is " << *string << std::endl;
  ...
  default:
    ...
    if (string_to_float(string, &val) != -1)
    {
...
int Parser::insert_gen_rec(GenExpr * gen_expr, TreeExpr * root)
{
  ...
  if (insert_gen_rec(gen_expr, root->left) == -1)
...
InitCond * Parser::parse_init_cond(std::istream &  fs, char * name, MilkdropPreset * preset)
{
  ...
  {
    if ((parse_int(fs, (int*)&init_val.int_val)) == -1)
    {
      if (PARSE_DEBUG) printf("parse_init_cond: error parsing integer!\n");
      return NULL;
    }
  ...
  {
    if ((parse_float(fs, (float*)&init_val.float_val)) == -1)
    {
