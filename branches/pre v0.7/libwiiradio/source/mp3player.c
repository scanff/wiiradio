...
// Modified by scanff to handle a callback that can be used to grab decoded samples
#include <asm.h>
#include <processor.h>
#include <stdlib.h>
#include <stdio.h>
...
s32 LWR_MP3Player_PlayBuffer(const void *buffer,s32 len,void (*filterfunc)(struct mad_stream *,struct mad_frame *),
...
s32 LWR_MP3Player_PlayFile(void *cb_data,s32 (*reader)(void *,void *,s32),void (*filterfunc)(struct mad_stream *,struct mad_frame *),
void (*fftin)(s16* data,int len))
{
	if(thr_running==TRUE) return -1;
...
static void *StreamPlay(void *arg)
{
	...
	while(atend==FALSE && thr_running==TRUE) {
		if(Stream.buffer==NULL || Stream.error==MAD_ERROR_BUFLEN) {
			...
			//if (ReadSize > 0) // when doing a network stream there's really no end of data
