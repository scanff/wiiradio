...
Renderer::Renderer(Settings* pSettings, BeatDetect *beatDetect) :
	title_fontURL(pSettings->titleFontURL),
    menu_fontURL(pSettings->menuFontURL),
    presetURL(pSettings->presetURL),
    m_presetName("None"),
    vw(pSettings->windowWidth),
    vh(pSettings->windowHeight),
    texsize(pSettings->textureSize),
    mesh(pSettings->meshX, pSettings->meshY)
{
	int height = pSettings->windowHeight;
	int width = pSettings->windowWidth;

#ifdef _WII_
    WII_Init_GX(pSettings->textureSize,pSettings->textureSize); // -- GX Setup before render starts
#endif
    ...
    v = ( float * ) wipemalloc ( size * sizeof ( float ) );


    for (int j = 0; j < mesh.height - 1; j++)
	{
		int base = j * mesh.width * 2 * 3;
		...
		for (int i = 0; i < mesh.width; i++)
		{
			...
			v[strip] = mesh.identity[index].x;
			v[strip + 1] = mesh.identity[index].y;
			v[strip + 2] = 0;

			v[strip + 3] = mesh.identity[index2].x;
			v[strip + 4] = mesh.identity[index2].y;
			v[strip + 5] = 0;
...
void Renderer::SetupPass1(const Pipeline &pipeline, const PipelineContext &pipelineContext)
{
    ...
    WII_Renderer_Reset();
    GX_SetViewport(0, 0, renderTarget->texsize, renderTarget->texsize,0,1);
 	guMtxIdentity(textureview);
    guMtxIdentity(projection);
    guMtxIdentity(modelview);


    guOrtho(projection,0.0, 1.0, 0.0, 1.0, 0, 40);
...
void Renderer::Pass2(const Pipeline &pipeline, const PipelineContext &pipelineContext)
{
    ...
    GX_SetViewport(0, 0, this->vw, this->vh,0,1);
    guMtxIdentity(projection);
	guOrtho(projection,0.5, -0.5, -0.5, 0.5, 0, 40);
	GX_LoadProjectionMtx(projection, GX_ORTHOGRAPHIC);
    GX_LoadPosMtxImm(modelview, GX_PNMTX0);

	GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA, GX_BL_INVSRCALPHA, GX_LO_CLEAR);

	WII_SetLineWidth(this->renderTarget->texsize < 512 ? 1 : this->renderTarget->texsize / 512.0,GX_TO_ZERO);

	CompositeOutput(pipeline, pipelineContext);

   // guMtxIdentity(modelview);
   // guMtxIdentity(textureview);
    //guMtxTransApply(modelview, modelview, -0.5, -0.5, 0.0); // -- only if drawing overlay
...
{
#ifdef _WII_
    switch(pipeline.textureWrap)
	{
	    case false:
            WII_SetTextureAttrib(GX_CLAMP);
            break;
        default:
            WII_SetTextureAttrib(GX_REPEAT);
            break;
	}
	...
	if (pipeline.staticPerPixel)
	{
		for (int j = 0; j < mesh.height - 1; j++)
		{
			...
			for (int i = 0; i < mesh.width; i++)
			{
				...
				p[strip + 2] = pipeline.x_mesh[i][j+1];
				p[strip + 3] = pipeline.y_mesh[i][j+1];
	...
	else
	{
        ...
        for (int j = 0; j < mesh.height - 1; j++)
        {
            ...
            for (int i = 0; i < mesh.width; i++)
            {
                ...
                p[strip + 2] = mesh.p[index2].x;
                p[strip + 3] = mesh.p[index2].y;
    ...
    WII_SetColors(255,255,255,pipeline.screenDecay * 0xff);
    GX_SetBlendMode(GX_BM_BLEND, GX_BL_SRCALPHA,GX_BL_ZERO, GX_LO_CLEAR);

    guMtxIdentity(textureview);

    for (int j = 0; j < mesh.height - 1; j++)
        WII_VertArrays(GX_TRIANGLESTRIP,v,p,0,j* mesh.width*2,mesh.width*2,3);
...
void Renderer::reset(int w, int h)
{
    ...
    WII_SetTextureAttrib(GX_LINEAR);
...
void Renderer::CompositeOutput(const Pipeline &pipeline, const PipelineContext &pipelineContext)
{
    ...
    WII_VertArrays(GX_TRIANGLEFAN, &points[0][0], &tex[0][0],0,0,4,2);
