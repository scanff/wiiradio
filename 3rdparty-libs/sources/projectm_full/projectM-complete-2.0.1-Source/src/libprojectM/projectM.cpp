...
void projectM::readSettings (const Settings & settings )
{
    ...
    _settings.presetDIR = settings.presetDIR;
    projectM_init ( _settings.meshX, _settings.meshY, _settings.fps,
                    _settings.textureSize, _settings.windowWidth,_settings.windowHeight);
...
//        assert(pcm());
        ...
        this->renderer = new Renderer ( &_settings, beatDetect);
        running = true;
    ...
    int projectM::initPresetTools(int gx, int gy)
    {
        ...
        srand ( time ( NULL ) );
       if (_settings.presetDIR.rfind('/') == _settings.presetDIR.size() - 1)
            _settings.presetDIR.erase(_settings.presetDIR.rfind('/'));
        //std::string url = (m_flags & FLAG_DISABLE_PLAYLIST_LOAD) ? std::string() : settings().presetURL;
        std::string url = (m_flags & FLAG_DISABLE_PLAYLIST_LOAD) ? std::string() : _settings.presetDIR;
...
*/
        std::string preset;
       // if (_settings.presetURL.empty()) {
        //    preset = PresetFactory::IDLE_PRESET_PROTOCOL + "://" + IdlePresets::IDLE_PRESET_NAME;
        //} else {
        preset = _settings.presetDIR + "/" + _settings.presetURL;
        //}
